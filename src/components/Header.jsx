...
const Header = () => {
    ...
    return (
        <header className={` ${navbar === true ? ' backdrop-blur-sm  bg-black bg-opacity-60 transition-all duration-700 border-b-neutral-80 border-b-[1px]' : 'bg-transparent'} fixed top-0 w-full h-16 md:w-full max-md:h-14 z-40 `}>
            <div className='container mx-auto px-2 flex items-center h-full w-full'>
                <Link to={'/'}>
                    <img className='h-16 w-24 max-md:w-24 max-md:h-14 hover:scale-105 transition-all active:scale-90'
                        src='/images/logo.png'
                        alt='logo'
                    />
                </Link>
                ...
                <div className='flex ml-auto gap-7'>
                    <form className=' flex items-center relative pr-5  ' onSubmit={handleSubmit}>
                        <input className='  max-lg:hidden bg-transparent pr-4 py-1 outline-none w-44 placeholder:text-neutral-100  ' type='text' placeholder='Search here..' value={inputValue} onChange={handleInputValue}></input>
